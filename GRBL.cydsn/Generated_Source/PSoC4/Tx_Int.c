...
{
    CyRamVectors[CYINT_IRQ_BASE + Tx_Int__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + Tx_Int__INTC_NUMBER];
}
...
void Tx_Int_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((Tx_Int__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *Tx_Int_INTC_PRIOR = (*Tx_Int_INTC_PRIOR & (uint32)(~Tx_Int__INTC_PRIOR_MASK)) |
...
uint8 Tx_Int_GetPriority(void)
{
    ...
    priority = (*Tx_Int_INTC_PRIOR & Tx_Int__INTC_PRIOR_MASK) >> priorityOffset;
